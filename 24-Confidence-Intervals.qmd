...
> **Exercise**: If we want to be very certain we capture the population parameter, should we use a wider interval or a smaller interval?[^24-confidence-intervals-1]

[^24-confidence-intervals-1]: If we want to be more certain we will capture the fish, we should use a wider net. Likewise, we use a wider confidence interval if we want to be more certain that we capture the parameter. A higher level of confidence implies a wider interval.
...
We could also use `R` to find the confidence interval and conduct the hypothesis test. Notice that the connection between confidence intervals and hypothesis testing is reinforced here; we can use the `t_test()` function to conduct a $t$-test and to calculate a confidence interval.[^24-confidence-intervals-2] Read about the function `t_test()` in the help menu to determine why we used the `mu` option.

[^24-confidence-intervals-2]: We could also find a bootstrap confidence interval here. Then, if the bootstrap distribution looks like a $t$ distribution, we can use a $t$ interval with the bootstrap estimate of the standard error.
...
> **Exercise**: By how much do we have to increase the sample size in order to reduce the margin of error by half?[^24-confidence-intervals-5]

[^24-confidence-intervals-5]: We need to multiply the sample size by four in order to reduce the standard error, and hence, the margin of error, by half. This is because $\sqrt{n}$ is in the denominator of the standard error. If we wanted to reduce the margin of error to one-third of its value, we would need to multiply the sample size by nine.
