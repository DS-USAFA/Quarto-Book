...
## Transitioning from probability models to statistical models
...
#| fig-height: 2
#| fig-width: 3
#| fig-cap: "A graphical representation of probability and statistics. In probability, we describe what we expect to happen if we know that underlying process; in statistics, we don't know the underlying process, and must infer based on representative samples."
...
@fig-lik1 is a plot of the likelihood function as a function of the unknown parameter $\pi$.

```{r}
#| label: fig-lik1
#| echo: false
#| fig-cap: "Likelihood function for 18 successes in 50 trials"
...
By visual inspection, the value of $\pi$ that makes our data most likely, maximizes the likelihood function, is something a little less than 0.4, the actual value is 0.36 as indicated by the blue line in @fig-lik1.
...
When obtaining MLEs, there are times when analytical methods (calculus) are not feasible or not possible. In the Pruim book [@pruim2011foundations], there is a good example regarding data from Old Faithful at Yellowstone National Park. We leave the example to the interested learner.
...
The `faithful` data set is preloaded into `R` and contains 272 observations of 2 variables: eruption time in minutes and waiting time until next eruption. If we plot eruption durations, we notice that the distribution appears bimodal (see @fig-hist171).

```{r}
#| label: fig-hist171 
#| #| echo: false 
#| #| fig-cap: "Histogram of eruption durations of Old Faithful." 
...
```{r hist172-fig,fig.cap=}
#| fig-cap: "Histogram of eruption duration with estimated mixture of normals plotted on top."
