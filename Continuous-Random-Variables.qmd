...
Let's start by generating random samples from $X$. We can do this using a method called rejection sampling. Rejection sampling involves generating candidate points from a proposal distribution (usually a uniform distribution, with values between 0 and 1) and accepting them with a certain probability that ensures the desired distribution. To determine the probability of rejection, we essentially check whether a second random number $u$ is less than or equal to a specific function value. For example, if we want values where $y = 2x$, we'll check whether $u\leq 2x$. An alternative method for this simulation uses the inverse transform method, which requires deriving the cdf.[^continuous-random-variables-2] Because we have de-emphasized the cdf in this book, we'll focus on the rejection sampling method.
...
#   and uniform random numbers for acceptance
# generate more points than needed (10000*2) to ensure 
#   enough accepted samples
candidates <- runif(num_sim*5, 0, 1)
u <- runif(num_sim*5, 0, 1)
