...
Why not use $\binom{13}{2}$ instead of $\binom{13}{1}\binom{12}{1}$?[^08-probability-rules-2]

[^08-probability-rules-2]: Because this implies the order selection of the ranks does not matter. In other words, this assumes that for example 3 Kings and 2 fours is the same full house as 3 fours and 2 Kings. This is not true so we break the rank selection about essentially making it a permutation.
...
a.  $\mbox{P}(A\cup B)$\
b.  $\mbox{P}(A\cup B \cup C)$\
c.  $\mbox{P}(B'\cap C')$\
d.  $\mbox{P}(A\cup (B\cap C))$\
e.  $\mbox{P}((A\cup B \cup C)\cap (A\cap B \cap C)')$
...
4.  Consider the license plate example from the reading.
...
5.  Consider the party example in the reading.
...
6.  Consider the cards example from the reading.
...
7.  Advanced Question: Consider rolling 5 dice. What is the **probability** of a pour resulting in a full house?

## [Solutions Manual](https://ds-usafa.github.io/CPS-Solutions-Manual/PROBRULES.html) {.unnumbered}
