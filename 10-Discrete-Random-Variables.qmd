...
5.  Prove the Lemma from the Notes: Let $X$ be a discrete random variable, and let $a$ and $b$ be constants. Show $\mbox{E}(aX + b)=a\mbox{E}(X)+b$.

 

6.  We saw that $\mbox{Var}(X)=\mbox{E}[(X-\mu_X)^2]$. Show that $\mbox{Var}(X)$ is also equal to $\mbox{E}(X^2)-[\mbox{E}(X)]^2$.

## [Solutions Manual](https://ds-usafa.github.io/CPS-Solutions-Manual/RANDVAR.html) {.unnumbered}
