...
Let's consider a standard six-sided die and define a random variable $X$ as the result of a single die roll. The sample space is $S = \{1, 2, 3, 4, 5, 6\}$ and each outcome has an equal probability of $1/6$. The expected value of $X$ is easily found to be $E(X) = 3.5$.[^sampling-distributions-2]

[^sampling-distributions-2]: $E(X) = 1*{1\over 6} + 2*{1\over 6} + 3*{1\over 6} + 4*{1\over 6} + 5*{1\over 6} + 6*{1\over 6} = 3.5$
...
[^sampling-distributions-4]: In fact, we've actually created a **null distribution** by sampling 1,001 possible fair redistricting maps. This null distribution is developed by sampling under the assumption of fairness, the null hypothesis in this scenario.
...
> **Exercise:** Write such a law, using the simulation method described in this lesson. Specifically, fill in the blank: “If your map has more Democrat/Republican leaning districts than \_\_% of simulations, it is unfair and should be rejected.” What number would you choose? Why?[^sampling-distributions-6]
