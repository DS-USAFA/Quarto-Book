...
The value 149 corresponds to the number of emails in the data set that are spam *and* had no numbers listed in the email. Row and column totals are also included. The **row totals** provide the total counts across each row (e.g. $149 + 168 + 50 = 367$), and **column totals** are total counts down each column. The row and column totals are known as **marginal**[^06-categorical-data-2] counts (hence, `margins = TRUE`) and the values in the table are known as **joint**[^06-categorical-data-3] counts.

[^06-categorical-data-2]: Marginal counts are counts based on only one of the variables in a contingency table. For example, there are 367 spam emails in the table.

[^06-categorical-data-3]: Joint counts are counts based on both variables in a contingency table. For example, there are 149 emails that are spam *and* contain no numbers.
...
> **Exercise**:\
> In the table you just created, what does 0.748 represent?[^06-categorical-data-4]

[^06-categorical-data-4]: This is the proportion of `not spam` emails that had a small number in it.
...
> 1. Make `format` into a categorical factor variable. The levels should be "text" and "HTML".[^06-categorical-data-5]\
> 2. Create a contingency table from the `email` data set with `format` in the columns and `spam` in the rows.

[^06-categorical-data-5]: From the help menu on the data, HTML is coded as a 1.
...
> **Exercise**:\
> Create two tables with `number` and `spam`: one where `number` is in the columns, and one where `spam` is in the columns. Which table would be more useful to someone hoping to identify spam emails based on the type of numbers in the email?[^06-categorical-data-6]

[^06-categorical-data-6]: The table with `number` in the columns will probably be most useful. This table makes it easier to see that emails with small numbers are spam about 5.9% of the time (relatively rare). In contrast, we see that about 27.1% of emails with no numbers are spam, and 9.2% of emails with big numbers are spam.
...
> *Example*:\
> Examine both of the segmented bar plots. Which is more useful?[^06-categorical-data-7]

[^06-categorical-data-7]: @fig-barseg61 contains more information, but @fig-barseg62 presents the information more clearly. This second plot makes it clear that emails with no number have a relatively high rate of spam email -- about 27%! On the other hand, less than 10% of emails with small or big numbers are spam.
...
## [Solutions Manual](https://ds-usafa.github.io/CPS-Solutions-Manual/CATDATA.html) {.unnumbered}
