# Point Estimates and Sampling Variability {#sec-SAMPVAR}
...
For the purposes of this book, we'll create a population with only 2.5 million individuals. In this simulation, the sample gave a point estimate of $\hat{p}^1 = 0.893$. We know the population proportion for the simulation was $p = 0.88$, so we know the estimate had an error of $0.893 - 0.88 = +0.013$.

```{r}
# 1. Create a set of 2.5 million entries, where 88% of them are "support" and 12% are "not". 
pop_size <- 2500000
...
One simulation isn't enough to get a great sense of the distribution of estimates we might expect in the simulation, so we should run more simulations. In a second simulation, we get $\hat{p}^2 = 0.885$, which has an error of $+0.005$. In another, $\hat{p}^3 = 0.878$ for an error of $-0.002$. And in another, an estimate of $\hat{p}^4 = 0.859$ with an error of $-0.021$. With the help of a computer, we've run the simulation 10,000 times and created a histogram of the results from all 10,000 simulations in @fig-hist-support.

```{r}
#| cache: true 
# 4. Repeat steps 2 and 3 10,000 times.
set.seed(2024)
solar_results <- do(10000)*sum(sample(possible_entries, size = 1000) == "support") / 1000
```

```{r}
mean(solar_results$sum)
sd(solar_results$sum)
```
...
solar_results %>% 
  ...
  gf_vline(xintercept = mean(results$sum)) %>% 
...
Let's consider a standard six-sided die and define a random variable $X$ as the result of a single die roll. The sample space is $S = \{1, 2, 3, 4, 5, 6\}$ and each outcome has an equal probability of $1/6$. The expected value of $X$ is easily found to be $E(X) = 3.5$.[^18b-sampling-distributions-2]

[^18b-sampling-distributions-2]: $E(X) = 1*{1\over 6} + 2*{1\over 6} + 3*{1\over 6} + 4*{1\over 6} + 5*{1\over 6} + 6*{1\over 6} = 3.5$
...
```{r}
# 1. Create a population/sample space of the six values on a standard six-sided die
...
Again, one simulation isn't enough to understand the distribution of means we might expect, so we should run more simulations. In a few further simulations, we get $\bar{x}_2 = 3$, $\bar{x}3 = 4.8$*,* $\bar{x}3 = 2.4$, and $\bar{x}_4 = 2.6$. With the help of a computer, we've run the simulation 10,000 times and created a histogram of the results from all 10,000 simulations in @fig-hist-die.

```{r}
#| cache: true 
# 4. Repeat steps 2 and 3 10,000 times.
set.seed(2024)
dice_results <- do(10000)*mean(sample(possible_values, size = 5, replace = T))
```

```{r}
mean(dice_results$mean)
sd(dice_results$mean)
```
...
dice_results %>% 
  ...
  gf_vline(xintercept = mean(results$mean)) %>% 
  gf_lims(y = c(0, 1100)) %>% 
...
To get a real sense of how gerrymandering works, try it out yourself! Use [this simulation](http://polytrope.com/district) to practice gerrymandering, but stop after the 6th challenge.
...
![](figures/Virginia.PNG){#virginia-actual}
...
![](figures/Virginia_sample1.PNG){#virginia-sample1}
...
After adding 1,000 samples (for a total of 1,001 samples), we have results that look similar to those shown in @fig-virginia-res1000. What we've done here is create a **sampling distribution** for the number of Democrat-leaning districts in the state of Virginia by sampling 1,001 redistricting maps.[^18b-sampling-distributions-4] In the applet, we're able to compare the actual number of Democrat-leaning districts to our sampling distribution. Virginia appears to have a fair map. The legislature's map projects seven Democrat-leaning districts, and the vast majority of simulations (\$664/1001 = 66.3%) also had seven Democrat-leaning districts or fewer. So, the actual Virginia map is not unusual. There is little evidence that Republicans gerrymandered the state.

[^18b-sampling-distributions-4]: In fact, we've actually created a **null distribution** by sampling 1,001 possible fair redistricting maps. This null distribution is developed by sampling under the assumption of fairness, the null hypothesis in this scenario.
...
> **Exercise:** Write such a law, using the simulation method described in this lesson. Specifically, fill in the blank: “If your map has more Democrat/Republican leaning districts than \_\_% of simulations, it is unfair and should be rejected.” What number would you choose? Why?[^18b-sampling-distributions-6]

[^18b-sampling-distributions-6]: This question is building towards a hypothesis test. If the proportion of districts more extreme than the drawn map (the “p-value”) is lower than 5% (the “alpha level”), we reject the map (reject the null hypothesis that the map was fair). Of course, the analogy to hypothesis tests isn’t perfect, but it’s all about building intuition.
