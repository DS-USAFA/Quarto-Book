...
a.  Find $\mbox{E}(X)$ and $\mbox{E}(Y)$.\
b.  Find $\mbox{Var}(X)$ and $\mbox{Var}(Y)$.\
c.  Find $\mbox{Cov}(X,Y)$ and $\rho$. Are $X$ and $Y$ independent?\
d.  Find $\mbox{Var}(3X+2Y)$.
...
d.  Find $\mbox{Var}\left(\frac{X}{2}+2Y\right)$.

```{=html}
<!-- -->
```
3.  Suppose $X$ and $Y$ are *independent* random variables. Show that $\mbox{E}(XY)=\mbox{E}(X)\mbox{E}(Y)$.
...
6.  ADVANCED: Let $X_1,X_2,...,X_n$ be independent, identically distributed random variables. (This is often abbreviated as "i.i.d."). Each $X_i$ has mean $\mu$ and variance $\sigma^2$ (i.e., for all $i$, $\mbox{E}(X_i)=\mu$ and $\mbox{Var}(X_i)=\sigma^2$).
...
## [Solutions Manual](https://ds-usafa.github.io/CPS-Solutions-Manual/MULTIEXP.html) {.unnumbered}
